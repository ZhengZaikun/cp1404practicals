...
def main():
    ...
    for line in in_file:
        ...
        is_reflection = parts[2]
        # Construct a ProgrammingLanguage object using the elements
        # year should be an int
        language = ProgrammingLanguage(parts[0], parts[1], is_reflection, int(parts[3]), parts[4])
...
def using_csv():
    ...
    for row in reader:
        language = ProgrammingLanguage(row[0], row[1], row[2], int(row[3]), row[4])
        print(language)
    in_file.close()
